...
class LotteryApp:
    ...
    def load_prizes(self):
        ...
        try:
            with open("prizes.csv", "r", encoding="utf-8") as f:
                ...
                for line_num, row in enumerate(reader, start=2):  # 从第2行开始计数
                    try:
                        name = row['name'].strip()
                ...
                if error_lines:
                    messagebox.showwarning("数据问题", 
                        f"发现{len(error_lines)}处错误：\n" + "\n".join(error_lines[:3]))
    ...
    def roll(self):
        if self.is_rolling and time.time() - self.last_update > 0.05:
            prize = random.choice([p for p in self.prizes if p["quantity"] > 0])
            self.result_label.config(
                text=prize["name"],
