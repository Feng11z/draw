...
class RoundedButton(tk.Canvas):
    ...
    def _adjust_color(self, hex_color, factor):
        ...
        return "#{:02x}{:02x}{:02x}".format(
            *[min(int(c * factor), 255) for c in rgb]
        )
...
class LotteryApp:
    ...
    def _init_ui(self):
        ...
        # 奖品列表卡片
        list_card = ttk.Frame(main_frame, style='TFrame', 
                            relief='solid', borderwidth=2)
        list_card.pack(fill=tk.BOTH, expand=True, pady=10)
        
        # 列表标题
        ttk.Label(list_card, text="奖品库存", 
                font=StyleConfig.FONT_TITLE,
                background=StyleConfig.BG_CARD).pack(pady=5)
        
        # 带滚动条的列表
        scrollbar = ttk.Scrollbar(list_card)
        self.listbox = tk.Listbox(list_card, 
        ...
        ttk.Button(btn_frame, text="中奖记录", 
                 command=self.show_history).pack(side=tk.LEFT)
        
        # 底部信息
        ttk.Label(main_frame, text="技术支持: lanyu-cn.cn",
                cursor="hand2").pack(side=tk.BOTTOM)
